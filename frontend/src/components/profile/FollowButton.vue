...
import { follow, unfollow } from '@/api/generated/users/users'
...
const mutation = useMutation({
  mutationFn: (nextFollowing: boolean) =>
    nextFollowing ? follow(props.userId) : unfollow(props.userId),
  onSuccess: () => {
    emit('toggled')
  },
...
<template>
  <div class="follow-button">
    ...
    <span v-if="errorMessage" class="follow-button__error" role="alert">{{ errorMessage }}</span>
  </div>
</template>
