...
// Publicar agora fica na mesma tela ("Publicado!" — docs/telas/10-publicar.html
// #done) em vez de navegar direto para a publicação. Para inspecionar o
// conteúdo publicado, vamos para o feed e entramos na publicação pelo título.
async function publishAndOpen(page: Page, title: string): Promise<void> {
  await page.getByRole('button', { name: 'Publicar' }).click()
  await expect(page.getByRole('heading', { name: 'Publicado!' })).toBeVisible()
  await page.getByRole('button', { name: 'Ver no feed' }).click()
  await expect(page).toHaveURL('/')
  await page
    .locator('article', { hasText: title })
    .first()
    .getByRole('link', { name: 'Ver publicação' })
    .click()
...
test('criação de prato valida imagem e envia multipart', async ({ page }) => {
  ...
  await page.getByLabel('Como se chama?').fill('Prato criado no teste')
  await expect(page.getByAltText('Prévia da imagem selecionada')).toBeVisible()

  await publishAndOpen(page, 'Prato criado no teste')
  ...
  await expect(page.getByRole('button', { name: 'Reagir' })).toBeDisabled()
...
test('adiciona tags na publicação e elas aparecem na página de detalhe', async ({ page }) => {
  ...
  await page.getByLabel('Como se chama?').fill('Prato com tags')
  await page.locator('input[type="file"]').setInputFiles(image)
  ...
  await publishAndOpen(page, 'Prato com tags')
...
test('minha versão preserva a origem e pré-carrega a receita original', async ({ page }) => {
  ...
  await page.getByLabel('Como você vai chamar a sua?').fill('com cobertura cítrica')
  await page.locator('input[type="file"]').setInputFiles(image)
...
test('minha versão interrompe o formulário quando a origem não existe', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Publicar minha versão' })).toHaveCount(0)
})
...
test('rascunho é salvo ao sair, listado em Rascunhos e retomado para publicar', async ({
  ...
  await page.getByLabel('Como se chama?').fill('Rascunho de bolo')
  await page.locator('input[type="file"]').setInputFiles(image)
  await expect(page.getByAltText('Prévia da imagem selecionada')).toBeVisible()
  ...
  await expect(page.getByLabel('Como se chama?')).toHaveValue('Rascunho de bolo')
  await expect(page.getByAltText('Prévia da imagem selecionada')).toBeVisible()
